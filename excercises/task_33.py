...
(Under UNIX/Linux and Windows, something similar might exist.) Apart from the text to be spoken, your
procedure also needs to accept two additional parameters: a float indicating the length of the pause between each
spoken ICAO word, and a float indicating the length of the pause between each word spoken.
'''
...
def stringto_icao(a):
    ...
    for n in b:
        for i in n:
            result.append(d[i])
        result.append('WAIT=10')
    print(result)

stringto_icao('FASZ kalap')